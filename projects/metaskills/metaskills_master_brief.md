# Metaskills Recruitment — Master Brief
**IsArtifact: true**
**Project Type:** Speculative Outreach Build
**Client:** Metaskills Recruitment & Training Consulting
**Website:** https://www.metaskillsrecruitment.com/
**Lead:** Neelanjana Choudhury, Founder & Chief Talent Strategist
**Location:** Bengaluru, India | Founded: 2015 | All-woman team

---

## Phase 1: Brand & Content Audit

### What They Do
Boutique recruitment consultancy specialising in **Leadership & IT hiring** for fast-growing startups. Also offer training/upskilling. Known for pre-vetted, pre-scanned candidates — reducing rejection rates and shortening hiring cycles.

### Sectors
IT, ITES, e-commerce, digital marketing, fintech/banking

### Notable Clients
Keka Software, Bosch, Incedo

### Awards
SME Business of the Year Award 2023

### Current Site Observations (inferred from research — site blocked to crawlers)
- Functional but likely standard recruitment agency aesthetic
- No clear bifurcation for employer vs. candidate visitor journeys
- Opportunity: elevate the brand to match the quality of their client roster

---

## Phase 1: Discovery Decisions

### Project Goal
**Lead Generation (Dual-Track)**
Generate employer inquiries AND candidate applications. Both audiences must feel seen immediately on landing.

### Site Architecture
**Single Page Website** — Navigation anchors scroll to sections.
No hard page transitions. Everything lives on one scrollable canvas.

### Target Audiences

| Audience | Who They Are | What They Need |
|---|---|---|
| **TG1: Employers** | HRs, CTOs, Founders at startups & mid-size firms | Fast, quality hires. Trust signals. A way to submit a vacancy. |
| **TG2: Job Seekers** | Mid-to-senior IT/leadership professionals | Reassurance the agency is legit. Roles that match their level. A way to register/apply. |

### The Bifurcation Mechanic
A **dual-CTA split** must be visible at or near the hero — no ambiguity about which path to take.
Proposed: "**I'm Hiring**" vs. "**I'm Looking for Work**" — each scrolling to or linking to their respective section.

### Primary Action (Employers)
Fill a quick vacancy brief / book a discovery call.

### Primary Action (Job Seekers)
Submit CV / register interest.

---

## Phase 1: Positioning Assumptions

### Brand Voice
Sharp, warm, credible. Boutique without being exclusive. Human without being casual.

### Differentiation Angle (Assumed)
- Boutique speed + quality vs. big-agency volume
- Founder-led accountability (not a faceless portal)
- Pre-vetted talent = employer's time is respected
- All-woman team — subtle brand warmth signal, not a headline

### What It Must NOT Feel Like
Corporate. Cluttered. Cold. Generic job board.

### Aesthetic Assumption
Clean-slate visual rebrand. Elevated, modern, slightly editorial.
Should feel like a premium consultancy — not a job portal.

---

## Phase 2: Competitive Research

| # | Competitor | Category | Hero Angle | CTA | Audience Split | Visual Tone |
|---|---|---|---|---|---|---|
| 1 | Korn Ferry | Global Aspirational | "A global firm that powers performance" | Contact for consultation | Employer-dominant | Premium, corporate, blue |
| 2 | Crescendo Group | Direct (India boutique) | "Leadership & Mid-Senior Hiring Experts" | Partner with us / Join talent network | Dual — explicit employer/candidate paths | Modern, approachable |
| 3 | Xpheno | Local / Better Rated (Bengaluru) | "Specialist Staffing" — speed, scale, quality, cost | Service inquiries / Contact | Employer-dominant, candidate secondary | Tech-forward, minimal |
| 4 | Spencer Stuart | Domain Best (leadership search) | "Executive Search, Board & Leadership Consulting" | Consultation (retainer-based) | Almost entirely employer-focused | Gravitas, serif, conservative |
| 5 | Randstad India | Country Best | "Leading Specialist Recruitment Agency" | Apply Now (candidate-first) | Genuinely balanced, candidate-friendly | Warm, human, people-photography |

### Key Patterns
- 4 of 5 competitors lead with employer messaging — candidates feel secondary
- No boutique firm in India has a distinctive personality — all generic corporate templates
- Generic positioning language everywhere: "connect," "talent," "leadership" — no specificity
- Crescendo is the most direct competitor and the only boutique with a clear dual-path — but still template-feeling

### White Space (What Nobody Is Doing Well)
1. **Authentic boutique warmth** — founder-led, human, not a faceless portal
2. **Balanced dual-audience narrative** — candidates treated as clients, not just pipeline
3. **Specific over generic** — no competitor states exactly what makes them different
4. **Visual differentiation** — everyone looks the same; room for an editorial, elevated aesthetic
5. **Bengaluru-first identity** — no competitor truly owns the local startup/tech ecosystem angle

### Forced Differentiation Decision
> **Metaskills = "The boutique that champions both sides."**
> Employers get pre-vetted quality and boutique speed. Candidates get respect, transparency, and genuine advocacy. The dual-tab architecture directly expresses this — both audiences land on equal, thoughtful experiences. No competitor at the boutique level does this.

---

### Site Architecture (Single Page, Scroll Navigation)

`Hero → Tabbed Audience Module → Why Metaskills → Social Proof → Contact/Footer`

**Tabbed Audience Module — the core UX mechanic:**
Sits directly below the first fold. A tab switcher — "I'm Hiring" / "I'm Looking for Work" — that renders two distinct mini-landing pages within the same section. Each tab contains its own value prop, process/how-it-works, and CTA. No scrolling required to switch audiences. The rest of the page (Why Metaskills, Social Proof, Contact) is shared.

| Tab | Content | CTA |
|---|---|---|
| I'm Hiring | Employer value prop, how the process works, trust signals | Submit a vacancy / Book a call |
| I'm Looking for Work | Candidate value prop, what to expect, sectors we place in | Register / Submit CV |

---

## Phases To Complete

- [x] Phase 1: Discovery & Audit
- [x] Phase 2: Competitive Research
- [x] Phase 3: Strategy & Blueprint
- [ ] Phase 4: Visual Direction
- [ ] Phase 5: Build (Pass 1 → Pass 2 → Framer)
